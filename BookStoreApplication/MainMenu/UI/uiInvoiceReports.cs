...
namespace GUI.UI
{
    public partial class uiInvoiceReports : UserControl
    {
        private readonly BookInvoiceBLL invoice;
        private string status = "year";
        public uiInvoiceReports()
        {
            InitializeComponent();
        ...
        private async void LoadTableYear()
        {
            ...
            binding.DataSource = await invoice.StatisticalReceiptByYearAsync(Convert.ToInt32(cbxY.Text.Trim()));
        ...
        {
            BindingSource binding = new BindingSource();
            binding.DataSource = await invoice.StatisticalReceiptByMonthAsync(Convert.ToInt32(cbxM.Text.Trim()),
        ...
        {
            BindingSource binding = new BindingSource();
            binding.DataSource = await invoice.StatisticalReceiptByQuaterAsync(Convert.ToInt32(cbxM.Text.Trim()),
                Convert.ToInt32(cbxY.Text.Trim()));
